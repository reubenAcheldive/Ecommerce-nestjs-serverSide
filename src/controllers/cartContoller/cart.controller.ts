...
export class CartController {
  ...
  async getCartShopping(@Param("customerRef") customerRef: string) {
    if (!customerRef)
      ...
      const cartResponse1 = await this.cartService.getCartByCustomerId(customerRef, 1);

      if (cartResponse1.length === 0) {
        const cartResponse2 = await this.cartService.getCartByCustomerId(customerRef, 2);
